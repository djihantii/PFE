function filterFile(listEquipes){
  ...
  d3.csv("data.csv", function(result, columns) {
    ...
    for(var i = 0; i<columns.length; i++){
      ...
      if (listEquipes.includes(columns[i].Equipe)) {
    ...
    // for (var j = 0; j< result.length; j++) alert('resulting '+result[j]].Equipe);
    for(var j = 0; j< result.length; j++) {
      alert('pushing'+result[j].Equipe);
    }
    return result;
...
function drawChart(listEquipes){
